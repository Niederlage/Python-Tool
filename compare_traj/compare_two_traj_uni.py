...
def plot_all(traj_b, traj_m, baseline_b, baseline_m, scalar_list, ave):
    ...
    ##################### draw  OpenVSLAM baseline distribution ########################################
    ax = fig.add_subplot(235)
    counts, bins = np.histogram(baseline_m, bins=70)
    ax.hist(bins[:-1], bins, weights=counts, facecolor='tab:blue', alpha=0.75)
    ax.set_ylabel("frequency", fontsize=12)
    ax.tick_params(axis='y')
...
def main():
    ...
    b_traj_cp = get_corresponding_frame(b_pose, m_pose, skip_fr=1) - start_bias
    b_traj_cp = blendertraj.rot_traj(b_traj_cp, -np.pi * 6 / 8)   # resident pi * 0/4 #
